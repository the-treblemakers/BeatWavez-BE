...
io.on("connection", (socket) => {
    ...
    socket.on("CREATE_ROOM", ({ stageName }) => {
        ...
        io.to(socket.id).emit("ROOM_JOIN_RESULT", {
            roomJoined: true,
            stageName,
            roomName,
            isHost: true,
            passcode: roomDirectory[roomName].passcode,
        });

        // WELCOME MESSAGE, SEND UPDATED ARRAY TO ROOM, AND CONSOLE LOG CREATE ROOM EVENT
        io.in(roomName).emit("UPDATE_MESSAGE_ARRAY", {
            stageName: `room`,
            message: `Welcome to the party in room ${roomName}!`,
        });
    ...
    socket.on("JOIN_ROOM", ({ stageName, roomName, inputPasscode }) => {
        ...
        if (inputPasscode !== roomDirectory[roomName].passcode) {
            ...
            roomDirectory[roomName].guestArray = [
                ...roomDirectory[roomName].guestArray,
                { stageName, socketId: socket.id },
            ];
    ...
    socket.on(
        "ADD_TO_QUEUE",
        ({ title, vidId, stageName, thumbnail, roomName }) => {
            console.log({ title, vidId, stageName, thumbnail, roomName });

            roomDirectory[roomName].queueArray = [
                ...roomDirectory[roomName].queueArray,
                { title, vidId, stageName, thumbnail },
            ];
            io.in(roomName).emit("UPDATE_QUEUE_ARRAY", {
                queueArray: roomDirectory[roomName].queueArray,
            });
        }
    );
    ...
    socket.on("UPDATE_ROOMS_ARRAY", () => {
        ...
        io.to(socket.id).emit("UPDATE_ROOMS_ARRAY", {
            roomsResults: Object.keys(roomDirectory),
        });
    });
});
