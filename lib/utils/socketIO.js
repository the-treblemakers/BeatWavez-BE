...
io.on('connection', (socket) => {
    ...
    socket.on('CREATE_ROOM', ({ stageName, roomName }) => {
        ...
        io.to(socket.id).emit('MESSAGE', { stageName: 'room', message: `Welcome to the ${roomName} room, ${stageName}!`, timeStamp: '' });
    });
    ...
    socket.on('JOIN_ROOM', ({ stageName, roomName }) => {
        ...
        io.to(socket.id).emit('MESSAGE', { stageName: 'room', message: `Welcome to the ${roomName} room, ${stageName}!`, timeStamp: '' });
        socket.broadcast.to(roomName).emit('MESSAGE', { stageName: 'room', message: `Welcome ${stageName} to the party!`, timeStamp: '' });
    });
