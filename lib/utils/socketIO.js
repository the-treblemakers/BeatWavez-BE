...
io.on('connection', (socket) => {
    ...
    socket.on('JOIN_ROOM', ({ stageName, roomName }) => {
        ...
        socket.join(roomName);
        socket.broadcast.to(roomName).emit('REQUEST_HOST', { sender: socket.id });  // requests hostId
        io.to(socket.id).emit('MESSAGE', { stageName: '-> room <-', message: `Welcome to the ${roomName} room, ${stageName}!`, timeStamp: new Date().toLocaleTimeString() });
    ...
    socket.on('ADD_TO_QUEUE', ({ title, vidId, stageName, thumbnail, hostId }) => {
        io.to(hostId).emit('ADD_TO_QUEUE', { title, vidId, stageName, thumbnail });
    });
