...
io.on('connection', (socket) => {
    ...
    socket.on('CREATE_ROOM', ({ stageName, roomName }) => {
        ...
        io.to(socket.id).emit('HOST', { hostId: socket.id, sender: socket.id });  // set self as host
        io.to(socket.id).emit('MESSAGE', { stageName: '-> room <-', message: `Welcome to the ${roomName} room, ${stageName}!`, timeStamp: new Date().toLocaleTimeString() });
    });

    socket.on('JOIN_ROOM', ({ stageName, roomName }) => {
        console.log(roomName, stageName, 'JOIN_ROOM');
        socket.join(roomName);
        socket.broadcast.to(roomName).emit('HOST', { hostId: null, sender: socket.id });  // requests host from users in room
        io.to(socket.id).emit('MESSAGE', { stageName: '-> room <-', message: `Welcome to the ${roomName} room, ${stageName}!`, timeStamp: new Date().toLocaleTimeString() });
        socket.broadcast.to(roomName).emit('MESSAGE', { stageName: '-> room <-', message: `Welcome ${stageName} to the party!`, timeStamp: new Date().toLocaleTimeString() });
    });
    ...
    socket.on('HOST', ({ hostId, sender }) => {
        io.to(sender).emit('HOST', { hostId: hostId, sender: null });
    });
